...
function drawRuler(count) {
  ...
  //刻度值数组
  let scaleValueList = [];
  for (let i = 0; i <= length.value; i += 10) {
  ...
  //清空画布
  cxt.clearRect(0, 0, length.value, height);
  ...
  if (initParams.background) {
    // cxt.fillStyle = '#ffa43c';
    // cxt.fillRect(0, 0, ruleWidth, ruleHeight);
  }
  ...
  for (let i = 0; i < length.value; i++) {
    ...
    if (props.direction === 'horizontal') {
      ...
      x = length.value - (division * i - count * division) - start - 10
    }
    cxt.moveTo(x, 0);
  ...
  scaleValueList.forEach(function (num, i) {
    ...
    cxt.fillText(num.toString(), props.direction === 'horizontal' ? x + 10 : (length.value - x - start - 10), Math.floor(initParams.height * 0.78));
  });
  cxt.closePath();
  ...
  if (props.highlight?.length > 0) {
    props.highlight?.filter(val => {
      ...
      if (props.direction === 'horizontal') {
        ...
        highlightX = length.value - highlightX
      }
