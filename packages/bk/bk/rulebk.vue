<template>
  ...
  transformOrigin: (width/4)+'px'+ ' '+ (width/4)+'px'
...
function drawRuler(count) {
  ...
  //清空画布
  cxt.clearRect(0, 0, width.value, initParams.height);
  ...
  for (let i = 0; i < width.value; i++) {
    ...
    if (props.direction === 'horizontal') {
      ...
      x = width.value - (division * i - count * division) - 10
    }
    cxt.moveTo(x, 0);
  ...
  scaleValueList.forEach(function (num, i) {
    ...
    cxt.fillText(num.toString(), props.direction === 'horizontal' ? x + 10 : (width.value - x - 10), Math.floor(initParams.height * 0.78));
  });
  cxt.closePath();
  ...
  if (props.highlight?.length > 0) {
    props.highlight?.filter(val => {
      ...
      if (props.direction === 'horizontal') {
        ...
        highlightX = width.value - highlightX
      }
